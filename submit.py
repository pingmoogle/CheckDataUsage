...
def main():
    ...
    csrfdata = re.search('<input type="hidden" name="_csrf" value="(.*)">		<div class="header">',
                         pageres.text).group(1)
    # verifyimgURL = "https://zfw.xidian.edu.cn" + re.search('<img id="loginform-verifycode-image" src="(.*)"
    # alt="">',pageres.text).group(1) verify_code = showimage(verifyimgURL) Post fromdata result = conn.post(
    # "https://zfw.xidian.edu.cn",data={"_csrf":csrfdata,"LoginForm[username]":rawdata["userID"],"LoginForm[
    # password]":rawdata["userPSWD"],"LoginForm[verifyCode]":verify_code,"login-button":NULL}) formdata =
    # parse.quote("_csrf="+csrfdata+"&LoginForm[username]="+rawdata["userID"]+"&LoginForm[password]="+rawdata[
    # "userPSWD"]+"&LoginForm[verifyCode]"+verify_code+"&login-button=") formdata = "_csrf="+ parse.quote(csrfdata) +
    # "&LoginForm%5Busername%5D="+rawdata["userID"]+"&LoginForm%5Bpassword%5D="+rawdata[
    # "userPSWD"]+"&LoginForm%5BverifyCode%5D="+verify_code+"&login-button="
    formdata = {
        "_csrf": csrfdata,
